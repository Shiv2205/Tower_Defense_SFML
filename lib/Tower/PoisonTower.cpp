...
PoisonTower::PoisonTower() 
    : Tower("PoisonTower", 1, 120, 9.0, 1.0, 6.0) {}

// Attack Method
void PoisonTower::attack() const {
    cout << getName() << " sprays poison! Deals "
         << getPower() << " damage over time!\n";
}
    ...
    for (auto &critter : critters) {
        if (!critter.isDead()) {
            ...
            if ((dx + dy) <= this->getRange()) {
                ...
                cout << getName() << " poisons critter at (" << critterPos.row << ", " << critterPos.col << ")!" << endl;
            }
        }
    }
}
